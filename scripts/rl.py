...
from alphagen.rl.policy import GPT2SharedNet 
from alphagen.rl.policy import TransformerSharedNet  # 修改import
...
class CustomCallback(BaseCallback):
    ...
    def _on_step(self) -> bool:
        ...
        if self.pbar is not None:
            self.pbar.update(1)
            if self.num_timesteps % 100 == 0:  # 每100步打印一次信息
                print(f"\nStep {self.num_timesteps}: Best IC = {self.pool.best_ic_ret:.4f}")
        return True
...
def run_single_experiment(
    ...
    tag = (
        ...
        f"llm_d{drop_rl_n}")
    name_prefix = f"{instruments}_{pool_capacity}_{seed}_{timestamp}_{tag}_{model}"
    ...
    if model == "gpt2":
        policy_kwargs=dict(
            features_extractor_class=GPT2SharedNet,  # 使用GPT2
            features_extractor_kwargs=dict(
                d_model=768,          # GPT2的默认维度
                dropout=0.1,
                device=device,
                pretrained=True,      # 使用预训练模型
                freeze_transformer=True,  # 冻结transformer块
                train_layernorm=True,    # 训练LayerNorm
                train_positional=True,   # 训练位置编码
            ),
        )
    elif model == "transformer":
        policy_kwargs=dict(
            features_extractor_class=TransformerSharedNet,  # 改用Transformer
            features_extractor_kwargs=dict(
...
def main(
    ...
    for s in random_seeds:
        ...
        end_t = time.time()
        print(f"Model: {model}")
        print(f"[Main] Finished training {steps} steps in {end_t - start_t:.2f} seconds")
